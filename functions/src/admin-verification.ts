 ...
 *
 * `admin/_adminGuard.ts` 의 `ADMIN_REGION` 와 동일.
 */
...
export const adminListProvisionalProfiles = functions
  .region(REGION)
  .https.onCall(
    async (_data, context) => {
...
export const adminListVerificationReviewHistory = functions
  .region(REGION)
  .https.onCall(
    async (_data, context) => {
      await requireAdmin(context);
      ...
      try {
        ...
        for (const doc of profileSnap.docs) {
          ...
          items.push({
            ...
            ownerName: String(
              (data as Record<string, unknown>).ownerName ?? ""
            ),
            address: String((data as Record<string, unknown>).address ?? ""),
            bizNo: String(bv["bizNo"] ?? ""),
...
export const adminSetProfileVerification = functions
  .region(REGION)
  .https.onCall(
    async (data, context) => {
...
export const requestBusinessNameReview = functions
  .region(REGION)
  .https.onCall(
    async (data, context) => {
      if (!context.auth) {
...
export const adminListBusinessNameReviewRequests = functions
  .region(REGION)
  .https.onCall(
    async (_data, context) => {
      await requireAdmin(context);
...
export const adminResolveBusinessNameReview = functions
  .region(REGION)
  .https.onCall(
    async (data, context) => {
      const adminUid = await requireAdmin(context);
