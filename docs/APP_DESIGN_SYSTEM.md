...
### 2.3 모바일 딥링크·서브 화면(참고)

`lib/core/router/app_router.dart`에 `/quiz`, `/books`, `/policy`, `/jobs`, 이력서·관리자 등이 정의되어 있다. 시안을 나눌 때 **「탭 안의 탭」**과 **「풀스크린 푸시」**를 구분하면 혼선이 줄어든다.
...
- 예외: 일부 **레거시/퍼블리셔** 화면은 그림자·라운드가 다를 수 있음 — 화면별 위젯 확인.
...
5. 웹 시안은 **이 문서만으로 불충분** — 웹 전용 스펙 파일을 병행할 것.
