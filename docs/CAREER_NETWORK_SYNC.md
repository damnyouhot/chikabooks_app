# 치과 네트워크 · 이력서 경력 동기화 (검증·배포)

이력서 경력을 커리어 카드(치과 네트워크)에 반영하는 기능은 **웹과 모바일이 같은 Flutter 화면**을 씁니다. 다만 **스토어에 올린 앱은 웹 배포만으로는 갱신되지 않습니다.**

## 화면에서 어디로 가나

1. 하단 **「커리어」** 탭 (앱) 또는 웹에서 **`/jobs`** 로 이동  
2. 상단 세그먼트에서 **「커리어 카드」** 선택 (채용이 아닌 쪽)  
3. **「나의 치과 네트워크」** 카드에서 항목을 탭하거나, 영역을 펼쳐 **치과 네트워크** 시트를 연다  
