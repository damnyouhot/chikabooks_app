...
///
/// 성능: StreamBuilder(실시간 리스너) → 1회 조회 + RefreshIndicator
/// 이 데이터는 실시간 변경이 거의 없으므로 1회 조회가 적절.
...
class _HiraUpdateSectionState extends State<HiraUpdateSection> {
  late Future<List<HiraUpdate>> _future;

  @override
  void initState() {
    super.initState();
    _future = HiraUpdateService.getAllUpdates();
  }
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: updates
                          .skip(3)
                          .map((update) =>
                              HiraUpdateCompactItem(update: update))
                          .toList(),
                    ),
                  ),
                ],
