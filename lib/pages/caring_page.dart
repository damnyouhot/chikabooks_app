...
class _CaringPageState extends State<CaringPage> with TickerProviderStateMixin {
  ...
  static const double _kSpeechOverlayExtraTop = 0.75 * (16.0 * 0.85 * 1.5);
  ...
  Future<void> _loadQuizCard() async {
    try {
      ...
      final schedule = await QuizPoolService.getTodaySchedule(
        contentConfig: cfg,
      );
      if (!mounted) return;
      final qs = <String>[];
  ...
  Future<void> _loadBookCard() async {
    try {
      final ebooks = await EbookService().fetchAllEbooks().catchError(
        (_) => <Ebook>[],
      );
      if (!mounted) return;
      Ebook? pick;
  ...
  void _onCircleTap() async {
    ...
    try {
      ...
      if (result.state != null) {
        ...
        final ok = await CaringStateService.saveStateSequential(result.state!);
        if (!mounted) return;
        if (!ok) {
          setState(() => _caringState = prev);
          _enqueueReaction('저장에 실패했어요. 연결을 확인하고 다시 시도해 주세요.');
        } else {
          AdminActivityService.log(
            ActivityEventType.tapCharacter,
            page: 'home',
          );
        }
      } else {
  ...
  void _onWash() async {
    ...
    setState(() => _caringPersistBusy = true);
    try {
      final result = await CaringActionService.tryWash(fromLocal: _caringState);
      if (!mounted) return;
      if (result.state != null) {
        setState(() => _caringState = result.state!);
        ...
        if (!ok) {
          ...
          AdminActivityService.log(
            ActivityEventType.washCharacter,
            page: 'home',
          );
        }
    ...
    try {
      if (_caringState.isSleeping) {
        ...
        if (wasAwake && sleeping.isSleeping) {
          ...
          _sleepFadeCtrl.forward();
          final ment =
              CaringMents.sleepStart[Random().nextInt(
                CaringMents.sleepStart.length,
              )];
          _enqueueReaction(ment);

          final ok = await CaringStateService.saveStateSequential(sleeping);
          if (!mounted) return;
          if (!ok) {
            setState(() => _caringState = prev);
            _sleepFadeCtrl.reverse();
        ...
        child: Stack(
          ...
          children: [
                          ...
                          children: [
                            ...
                            FadeTransition(
                              ...
                              child: _PolicyRollingCard(
                                ...
                                onTap: () {
                                  widget.onTabRequested?.call(2);
                                  widget.onGrowthSubTabRequested?.call(
                                    2,
                                    hiraInnerTab: 1,
                                  );
                                },
            ...
            if (isOnboarding && widget.onboardingDialogue != null)
              _buildOnboardingSpeechBubble(MediaQuery.of(context).size.height),
          ],
        ),
      ),
    );
  ...
  Widget _buildOnboardingSpeechBubble(double screenH) {
    final bubbleBottom =
        (_kBottomSectionH - _kCharacterTranslateY) +
                ...
                _dogController != null
                    ? LayoutBuilder(
                      builder: (ctx, constraints) {
                        ...
                        final scale = (screenH * kScreenRatio / availableH)
                            .clamp(0.85, 2.5);
            ...
            builder: (context, snap) {
              ...
              return Tooltip(
                ...
                child: InkWell(
                  ...
                  onTap: () => _showTreatGuideDialog(context),
                  child: Padding(
  ...
  Widget _buildBottomSection({bool isOnboarding = false}) {
    ...
    final blockCareActions = isSleeping || !_caringReady || _caringPersistBusy;
    final buttons = [
      _ActionBtn(
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [const _FloatingBone(phase: 0, top: 0, horizontalOffset: 0)],
      ),
    );
  }
}
      ...
      child: Material(
        ...
        child: InkWell(
          ...
          child: Container(
            ...
            child: LayoutBuilder(
              builder: (ctx, constraints) {
                final rightMaxW = (constraints.maxWidth * 0.55).clamp(
                  0.0,
                  200.0,
                );
                return Row(
      ...
      child: ScaleTransition(
        ...
        child: LayoutBuilder(
          builder: (ctx, _) {
            ...
            final fadeDuration =
                widget.suppressInteractionFade
                    ? Duration.zero
